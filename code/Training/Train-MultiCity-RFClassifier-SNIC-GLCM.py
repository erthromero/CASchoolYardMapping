...
# Last edit: 6/18/2024
#
# Code written to train a Random Forest classifier based on National Agricultural Imagery Program (NAIP) RGBN data.
...
# to be optimal based on processing capabilities of Mulford 222 workstation using ~2000 segments per 500m grid subset. 
# After segmentation and object statistics collection and labelling, the code then compiles these statistics for training a Random
# Forest Classifier from the sklearn library.
...
def segment_featuresR1(segment_pixels):
...
def main():
    ...
    output_model_path = "E:\\SummerGSR2024\\MLModels\\LA-Oak-Sac\\LA-Oak-Sac-SchoolYards-MoreStats-SNIC-GLCM-RF.joblib"
    ...
    for image_path, training_shp, subset_shp, city_name in  zip(image_paths, training_shapefile_paths, training_grid_paths, city_names):
        ...
        output_segments_path = f"E:\\SummerGSR2024\\ImSegmentation\\{city_name}\\Training\\RandomForest\\{city_name}TrainingSegments"
